...
#include "\x\x39\medicalretro\core\BlackOutSystem\infoArrayOffsets.hpp"
params [
...
_currentState = _unit getVariable VALSTR_BOS_STATE;

switch (_state) do
{
    case -1: {
        [_text] call X39_XMR_fnc_BOS_ChangeDisplayText;
        RETURN(true);
    };
    case 0: {
        if (_currentState > 1 && {!_override || ([_unit] call X39_XMR_fnc_BOS_CheckConditions)}) then
        {
            RETURN(false);
        }
        else
        {
            _unit setVariable [VALSTR_BOS_STATE, _state, true];
            [_unit] call X39_XMR_fnc_BOS_WakeUnit;
            RETURN(true);
        };
    };
    case 1: {
        ...
        else
        {
            ...
            _unit setVariable [VALSTR_BOS_INFO, _tmpArr];
            
            [_unit] call X39_XMR_fnc_BOS_BlackOutUnit;
            [_text] call X39_XMR_fnc_BOS_ChangeDisplayText;
    ...
    case 2: {
        if (_currentState > 2 && {!_override || ([_unit] call X39_XMR_fnc_BOS_CheckConditions)}) then
        {
            RETURN(false);
        }
        else
